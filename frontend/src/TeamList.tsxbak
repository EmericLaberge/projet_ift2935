...
function TeamList() {
  ...
  const apiEndpoint = 'http://127.0.0.1:6969';



  const handleEditClick = (id: GridRowId) => async () => {
    // query the user by id 
    const response = await fetch(`http://127.0.0.1:6969/teams/${id}`);
  ...
  async function handleViewClick() {
    ...
    if (isAllTeams) {
      ...
      setIsAllTeams(false);
      const response = await fetch(`http://127.0.0.1:6969/user_teams/${userId}`);
      if (!response.ok) {
      ...
      setIsAllTeams(true);
      // refetch all teams
      const response = await fetch(`http://127.0.0.1:6969/teams`);
      if (!response.ok) {
  ...
  const handleNewTeamSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch('http://127.0.0.1:6969/create_team', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    ...
    //   try {
    //     const response = await fetch('http://127.0.0.1:6969/user_teams');
    //     if (!response.ok) {
