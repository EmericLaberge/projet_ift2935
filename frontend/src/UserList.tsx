...
function UserList() {
  ...
  const handleEditClick = (id: GridRowId) => async () => {
    // query the user by id 
        const response = await fetch(`http://127.0.0.1:42069/users/${id}`);
    ...
    const handleDeleteUserSubmit = async (id: GridRowId) => {
      ...
      const response = await fetch('http://127.0.0.1:42069/users');
      if (!response.ok) {
    ...
    const handleNewUserSubmit = async (e: React.FormEvent) => {
      e.preventDefault();
      try {
        const response = await fetch('http://127.0.0.1:42069/create_user', {
          method: 'POST',
          headers: {
    ...
    const EditUserSubmit = async (user: User) => {
      try {
        const response = await fetch(`http://127.0.0.1:42069/users/${user.id}`, { method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
