...
function EventList() {
  ...
  async function handleViewClick() {
    ...
    if (isAllEvents) {
      ...
      const response = await fetch(`http://127.0.0.1:6969/user_events/${userId}`);
      if (!response.ok) {
      ...
      setIsAllEvents(true);
      // refetch all teams
      const response = await fetch(`http://127.0.0.1:6969/events`);
      if (!response.ok) {
  ...
  const handleEditClick = (id: GridRowId) => async () => {
    ...
    const response = await fetch(`http://127.0.0.1:6969/events/${id}`);
    const data: Event = await response.json();
    setEditEvent(data);
    setOpenEdit(true);
  ...
  const handleNewEventSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch('http://127.0.0.1:6969/create_event', {
        method: 'POST',
        headers: {
  ...
  const EditEventSubmit = async (event: Event) => {
    try {
      const response = await fetch(`http://127.0.0.1:6969/events/${event.id}`, {
        headers: {
          'Content-Type': 'application/json',
        },
